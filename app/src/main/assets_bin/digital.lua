local class,wordF,digitR,digitG,tonumtable,coipairs,fillunit,formatmode,numtableformat,tabletostr
class={}
...
digitG.get={function(r,g)--获取计数单位
  ...
  function(r,g) return (g+1)%2==1 and 1 or (g+1)/2+1 end,--亿进中数
  function(r,g) return g+1 end,}--万进日式
...
function fillunit(numtable,digitmode)--为数字数组填入计数单位
  local rindex,gindex,output=1,0,{}
...
formatmode={--不能略读单位名称的情况
  function(zcount,drlen,lv)return zcount<drlen*(2^(lv-1)) end,--上数
